...
export function Sidebar() {
  ...
  const isDark = settings.theme === 'dark' ||
    (settings.theme === 'system' && typeof document !== 'undefined' &&
    ...
    <aside className="hidden lg:flex flex-col w-60 min-h-screen bg-white dark:bg-gray-900 border-r border-[#EAE6DC] dark:border-gray-800 fixed left-0 top-0 bottom-0 z-30">
      ...
      <div className="p-6 border-b border-[#EAE6DC] dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-bold text-gray-900 dark:text-white">가계부</h1>
            <p className="text-xs text-gray-500 dark:text-gray-400">스마트 예산 관리</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-[#EAE6DC] dark:border-gray-800 space-y-2">
        ...
        <button
          onClick={toggleTheme}
          className="flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-gray-100 transition-colors"
        >
          {isDark ? (
            <>
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
